package calculator;

import org.junit.jupiter.api.Test;
import static org.junit.jupiter.api.Assertions.*;

class StringCalculatorShould {

    @Test
    void empty_string_should_return_0() {
        StringCalculator stringCalculator = new StringCalculator();
        assertEquals(0, stringCalculator.add(""));
    }

    @Test
    void string_with_single_number_should_return_number_as_int() {
        StringCalculator stringCalculator = new StringCalculator();
        assertEquals(1, stringCalculator.add("1"));
    }
    
    @Test
    void string_with_two_numbers_separated_by_comma_delimitor_should_return_their_sum() {
    	StringCalculator stringCalculator = new StringCalculator();
    	assertEquals(5, stringCalculator.add("2,3"));
    }
    
    @Test
    void string_with_two_numbers_separated_by_newline_delimitor_should_return_their_sum() {
    	StringCalculator stringCalculator = new StringCalculator();
    	assertEquals(10, stringCalculator.add("6\n4"));
    }
    
    @Test
    void string_with_three_numbers_separated_by_either_delimitor_should_return_their_sum() {
    	StringCalculator stringCalculator = new StringCalculator();
    	assertEquals(11, stringCalculator.add("6\n4,1"));
    }
    
    @Test
    void string_with_two_numbers_separated_by_given_delimitor_should_return_their_sum() {
    	StringCalculator stringCalculator = new StringCalculator();
    	assertEquals(3, stringCalculator.add("//;\n1;2"));
    }
}
